      ...
      <div className="mx-auto flex h-16 w-full max-w-[1280px] items-center justify-between px-4 lg:h-[65px] lg:px-4">
        ...
        {isLoggedIn ? (
          <div className="flex items-center gap-3">
            <Link
              href="/workspace"
              className="inline-flex h-9 items-center justify-center rounded-2xl bg-[linear-gradient(135deg,#4a3168_0%,#6f4d96_100%)] px-4 text-sm font-medium text-white hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4a3168]/20 lg:h-10 lg:px-6"
            >
              进入工作台
            </Link>
            <button
              className="inline-flex h-9 items-center justify-center rounded-2xl px-4 text-sm font-medium text-[#3f3f46] hover:-translate-y-0.5 hover:bg-[rgba(74,49,104,0.06)] hover:text-[#4a3168] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4a3168]/20 lg:h-10"
              type="button"
              onClick={() => {
                clearAuthSession();
                router.push("/login");
