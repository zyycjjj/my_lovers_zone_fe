...
import type {
  AuthMe,
  EntitlementStatus,
} from "@/domains/workspace/workspace-model";
...
const toolContinueMap: Record<
  ContentAsset["toolKey"],
  { label: string; nextTool: string }
> = {
...
function buildContinueHint(
  ...
  if (!asset) {
    if (streak >= 3)
      return `连续 ${streak} 天来开工了，选个目标开始今天的内容吧。`;
    return "先从一个明确的方向开始，生成后保存下来，明天就能接着用。";
  }
  if (stats && stats.todayCreated === 0) {
    const toolLabel =
      asset.title ||
      toolLabels[asset.toolKey as ContentAsset["toolKey"]] ||
      "内容";
    if (streak >= 2)
      return `上次做了「${toolLabel}」，连续 ${streak} 天了，今天继续？`;
  ...
  if (asset.status === "completed") {
    const toolLabel =
      ...
      "内容";
    if (streak >= 3)
      return `已完成「${toolLabel}」，这几天做得不少了，试试新方向？`;
...
function buildTodaySuggestion(
  ...
  if (streak >= 3 && todayCount === 0) {
    return {
      text: `连续 ${streak} 天开工了，今天选个目标出一条新的吧。`,
      actionLabel: "开始生成",
  ...
  if (streak >= 5) {
    return {
      text: `连续 ${streak} 天的老用户了，保持这个频率，一周下来效果很明显。`,
      actionLabel: "继续生成",
      actionHref: "/workspace",
    };
  }
  ...
  if (savedCount > 0 && completedCount === 0) {
    return {
      text: "你已有保存的内容，先把其中一条标记为完成，再继续生成新的。",
      actionLabel: "去完成",
      actionHref: "#assets",
    };
  }
  if (todayCount === 0 && quotaRemaining > 0) {
    return {
      text: "今天还没有生成内容，建议先生成一条能直接发出去的标题或脚本。",
      actionLabel: "开始生成",
      actionHref: "/workspace",
    };
  }
  if (quotaRemaining <= 2) {
    return {
      text: "剩余额度不多了，优先把最重要的内容先生成出来。",
      actionLabel: "开始生成",
      actionHref: "/workspace",
    };
  }
  return {
    text: "每天至少出一条，存下来就是自己的内容库。",
    actionLabel: "继续生成",
    actionHref: "/workspace",
  };
}
...
function buildWorkspaceContinueUrl(asset: ContentAsset): string {
  ...
  const keyword =
    asset.sourcePrompt?.trim() ||
    asset.title?.trim() ||
    asset.content.trim().slice(0, 60);
...
export default function MeScreen() {
  ...
  const [filterStatus, setFilterStatus] = useState<
    "all" | "saved" | "completed"
  >("all");
  const [filterTool, setFilterTool] = useState<ContentAsset["toolKey"] | "all">(
    "all"
  );
  // 展开的内容 ID 集合
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      apiRequest<{ items: ContentAsset[]; total: number }>(
        "/api/content-assets/me?limit=50"
      ),
      apiRequest<ContentStats>("/api/content-assets/stats/me").catch(
        () => null
      ),
  ...
  const displayName = useMemo(() => {
    if (!me) return "";
    return (
      me.account.displayName || me.account.phone || `用户 ${me.account.id}`
    );
  }, [me]);
  ...
  const savedAssets = useMemo(
    () => assets.filter((item) => item.status === "saved"),
    [assets]
  );
  ...
  async function handleComplete(assetId: number) {
    ...
    setActionMessage("");
    try {
      const updated = await apiRequest<ContentAsset>(
        `/api/content-assets/${assetId}/complete`,
        {
          method: "POST",
        }
      );
  ...
  const filteredAssets = useMemo(() => {
    ...
    if (filterTool !== "all")
      result = result.filter((a) => a.toolKey === filterTool);
    return result;
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] text-[#18181B]">
      ...
      <main className="mx-auto max-w-[1180px] px-4 py-6 sm:px-6 lg:px-8">
        <ErrorBoundary
          fallbackTitle="我的页出了点问题"
          fallbackDescription="内容记录暂时无法显示，刷新后重试。"
        >
          <section className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <div className="flex items-center gap-3">
                <span className="text-sm font-medium text-[#737378]">
                  {displayName}
                </span>
                {checkinStreak >= 2 ? (
          ...
          {pendingSummary?.count ? (
            <NoticePanel className="mb-6" tone="gold">
              你有 {pendingSummary.count} 笔订单待处理，最新订单状态为{" "}
              {pendingSummary.latest?.status || "pending"}。
            </NoticePanel>
          ) : null}
          ...
          <section className="grid gap-5 md:grid-cols-4">
            <Card className="rounded-[20px] border border-[rgba(0,0,0,0.08)] bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
              ...
              <div className="mt-2 text-sm text-[#737378]">
                {subscription?.expiredAt
                  ? `到期 ${formatDateTime(subscription.expiredAt)}`
                  : entitlement?.resetHint || "开通后获得生成额度"}
              </div>
            ...
            <Card className="rounded-[20px] border border-[rgba(0,0,0,0.08)] bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
              ...
              <div className="mt-2 text-[30px] font-semibold leading-none text-[#4A3168]">
                {quotaRemaining}
              </div>
              <div className="mt-4 h-2 overflow-hidden rounded-full bg-[#ECECF0]">
                <div
                  className="h-full rounded-full bg-[#D4668F]"
            ...
            <Card className="rounded-[20px] border border-[rgba(0,0,0,0.08)] bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
              ...
              <div className="mt-2 text-sm text-[#737378]">
                昨日 {contentStats?.yesterdayCreated ?? yesterdayAssets.length}{" "}
                条
              </div>
            </Card>
            <Card className="rounded-[20px] border border-[rgba(0,0,0,0.08)] bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
              ...
              <div className="mt-2 text-[30px] font-semibold leading-none text-[#4A3168]">
                {contentStats?.totalAll ?? assets.length}
              </div>
              <div className="mt-2 text-sm text-[#737378]">
            ...
            <Card className="rounded-[20px] border border-[rgba(0,0,0,0.08)] bg-white p-0 shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
              ...
              <div className="border-b border-[rgba(0,0,0,0.08)] px-5 py-4">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <div className="text-lg font-semibold text-[#27272A]">
                      内容记录
                    </div>
                ...
                {filteredAssets.length ? (
                  filteredAssets.map((asset) => {
                    ...
                    return (
                      <article
                        key={asset.id}
                        className="group px-5 py-4 transition-colors hover:bg-[#FAFAFA]/80"
                      >
                  ...
                  <div className="px-5 py-12 text-center">
                    ...
                    <div className="mt-3 flex items-center justify-center gap-2">
                      ...
                      <ButtonLink className="text-sm" href="/workspace">
                        去生成
                      </ButtonLink>
                    </div>
                  </div>
                    ...
                    <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-[#F5F3F7]">
                      ...
                      >
                        <path
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          d="M12 16v-5m0 0V7m0 5h5m-5 0H7l3-3"
                        />
                      </svg>
                    </div>
                    <div className="text-base font-semibold text-[#27272A]">
            ...
            <aside className="space-y-6">
              ...
              <Card className="rounded-[20px] border border-[rgba(0,0,0,0.08)] bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
                <div className="flex items-center justify-between">
                  ...
                  {(contentStats?.yesterdayCreated ?? yesterdayAssets.length) >
                  0 ? (
              ...
              <Card className="ui-card-dark p-5 text-white">
                <div className="text-lg font-semibold">任务小广场</div>
                <div className="mt-3 h-14 text-sm leading-7 text-white/80">
                  发张小卡片，看看别人今天在做什么，也给作品讨一句反馈。
                </div>
                <ButtonLink
                  className="mt-4 w-full bg-white text-[#4A3168] hover:bg-[#F5F3F7]"
                  href="/pricing"
                >
                  查看套餐
                </ButtonLink>
                <ButtonLink
                  className="mt-2 w-full bg-white/90 text-[#4A3168] hover:bg-[#F5F3F7]"
                  href="/community"
                >
                  去小广场
                </ButtonLink>
              ...
              <Card className="rounded-[20px] border border-[rgba(0,0,0,0.08)] bg-white p-5">
                ...
                <div className="mt-3 text-sm leading-7 text-[#737378]">
                  找一个愿意看你往前走的人。分享进展，不用一个人闷头冲。
                </div>
                <ButtonLink
                  className="mt-4 w-full bg-[#4A3168] text-white hover:bg-[#3D2856]"
                  href="/supporters"
                >
                  管理支持者
                </ButtonLink>
